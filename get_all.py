...
color_flags = ["c:%s" % c for c in colors]

all_color_flag_combos = [
    "c:%s c:%s -m:{%s/%s}" % (a, b, a, b) for a, b in itertools.combinations(colors, 2)
]
    ...
    for rarity_flag in ["rarity<rare", "rarity>=rare"]:
        ...
        for type_flag in [
            ...
            proc = subprocess.Popen(
                ["node", "./list-cards.js", search_term], stdout=subprocess.PIPE
            )
            ...
            if output_file_path:
                print("downloading!")
                subprocess.call(["./download-cards.sh", output_file_path])
