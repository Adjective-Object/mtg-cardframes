...
def percentile_mask(directory_path, percentile):
    ...
    if len(file_names) > FILE_LIMIT:
        print("%s files is too many images. Only loading first %s at once" % (
            len(file_names),
            FILE_LIMIT
        ))
    ...
    for [i, f] in enumerate(file_names):
        ...
        bump_mask = np.logical_and((composite_image_lab < next_image_lab), (counts < int_perecentile))
        bump_mask_reshaped = bump_mask.reshape((bump_mask.shape[0], bump_mask.shape[1], 1))

        # print(bump_mask.shape, bump_mask.min(), bump_mask.max())
        # plt.imshow(bump_mask * 1.0)
        # plt.show()

        composite_image_lab = next_image_lab * bump_mask + composite_image_lab * (1 - bump_mask)
        composite_image = next_image * bump_mask_reshaped + composite_image * (1 - bump_mask_reshaped)
